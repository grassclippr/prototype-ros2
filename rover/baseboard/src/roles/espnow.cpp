...
bool loadPeerFromNVS() {
    ...
    nvs.begin("espnow", false);
    size_t len = nvs.getBytes(NVS_PEER_KEY, peerInfo.peer_addr, 6);
    peerInfo.channel = nvs.getUChar(NVS_PEER_CHANNEL);
    nvs.end();

    if (len != 6 || (peerInfo.peer_addr[0] == 0x00 && peerInfo.peer_addr[1] == 0x00 &&
        peerInfo.peer_addr[2] == 0x00 && peerInfo.peer_addr[3] == 0x00 &&
        peerInfo.peer_addr[4] == 0x00 && peerInfo.peer_addr[5] == 0x00)) {
        printf("No valid peer found in NVS\n");
        return false;
    }
